...
- For broad read-only searches (where/what/how across many files, git history, or GitHub PRs/issues), delegate to the `explore-noprompt` subagent — it's steered to pre-approved `gh` readers and has no WebFetch, so it won't trigger permission prompts.
